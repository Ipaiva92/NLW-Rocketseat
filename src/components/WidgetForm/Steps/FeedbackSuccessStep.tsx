...
interface onFeedbackRestartRequestProps{
  onFeedbackRestartRequest: () => void;
}

export function FeedbackSuccessStep({onFeedbackRestartRequest}:onFeedbackRestartRequestProps) {
  return (
    <>
      <header>
        <CloseButton />
      ...
      <div className="flex flex-col items-center py-10 w-[304px]">
        ...
        <button onClick={onFeedbackRestartRequest} type="button" className="py-2 px-6 mt-6 bg-zinc-800 focus:ring-2 focus:ring-offset-2 focus:ring-offset-zinc-900 focus:ring-brand-500 rounded-md border-transparent text-sm leading-6 hover:bg-zinc-700 transition-colors">Quero enviar outro</button>
      </div>
    </>
  );
}
